...
//  private final ObjectRenderer leftEarObject = new ObjectRenderer();
  private final ObjectRenderer helmetObject = new ObjectRenderer();
...
//  private final float[] leftEarMatrix = new float[16];
  private final float[] helmetMatrix = new float[16];
...
//        noseObject.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, DEFAULT_COLOR);
        face.getRegionPose(RegionType.FOREHEAD_LEFT).toMatrix(helmetMatrix, 0);
        face.getRegionPose(RegionType.FOREHEAD_RIGHT).toMatrix(helmetMatrix, 0);
