...
//  private final ObjectRenderer leftEarObject = new ObjectRenderer();
  private final ObjectRenderer maskObject = new ObjectRenderer();
...
//  private final float[] leftEarMatrix = new float[16];
  private final float[] MOSMatrix = new float[16];
...
//      leftEarObject.setBlendMode(ObjectRenderer.BlendMode.AlphaBlending);
      maskObject.createOnGlThread(
              /*context=*/ this,
              "models/MOS.obj",
              "models/Mask_baseColor.png");
      maskObject.setMaterialProperties(0.0f, 1.0f, 0.1f, 6.0f);
      maskObject.setBlendMode(ObjectRenderer.BlendMode.AlphaBlending);
...
//        noseObject.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, DEFAULT_COLOR);
        face.getCenterPose().toMatrix(MOSMatrix, 0);
        maskObject.updateModelMatrix(MOSMatrix, scaleFactor);
        maskObject.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, DEFAULT_COLOR);
      }
    } catch (Throwable t) {
